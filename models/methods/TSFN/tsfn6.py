...
class Downsample(nn.Module):
    ...
    def forward(self, x, r):
        ...
        return rearrange(self.body(x), 'b c (h h1) (w w1) -> b (c h1 w1) h w', h1=2, w1=2), rearrange(self.body2(r), 'b c (h h1) (w w1) -> b (c h1 w1) h w', h1=2, w1=2)
...
class Upsample(nn.Module):
    ...
    def forward(self, x, r):
        return self.body(x), self.body2(r)
    def flops(self, inp_shape):
        C, H, W = inp_shape
        flops = 0
...
class Attention_2in(nn.Module):
    def __init__(self, dim, num_heads, bias, window_size=8, grid_size=8, window_size_dct=9,
        ...
        if proj_out:
            self.project_out = nn.Conv2d(dim, dim, kernel_size=1, bias=True)
            self.project_out2 = nn.Conv2d(dim, dim, kernel_size=1, bias=True)
        else:
            self.project_out = nn.Identity()
            self.project_out2 = nn.Identity()
    ...
    def forward(self, x, x2):
        ...
        cross_out2 = self.project_out2(cross_out2)
        return cross_out, cross_out2
...
class TransformerBlock_2in(nn.Module):
    def __init__(self, dim=32,
        ...
        if 'FLOPs' in cs:
            self.dct = DCT2_FLOPs(window_size_dct, window_size_dct)
            self.idct = IDCT2_FLOPs(window_size_dct, window_size_dct)
            self.idct2 = IDCT2_FLOPs(window_size_dct, window_size_dct)
        elif 'nodct' in cs:
            self.dct = nn.Identity()
            self.idct = nn.Identity()
            self.idct2 = nn.Identity()
        elif 'dct_torch' in cs:
            self.dct = DCT2x_torch()
            self.idct = IDCT2x_torch()
            self.idct2 = IDCT2x_torch()
        else:
            self.dct = DCT2x()
            self.idct = IDCT2x()
            self.idct2 = IDCT2x()
        ...
        if cs != 'identity':
            if norm_type[0] == 'InstanceNorm':
                norm1 = nn.InstanceNorm2d(dim)
                norm1_2 = nn.InstanceNorm2d(dim)
            elif norm_type[0] == 'LayerNorm':
                norm1 = LayerNorm(dim, LayerNorm_type) # , out_dir=i)
                norm1_2 = LayerNorm(dim, LayerNorm_type) # , out_dir=i)
            elif norm_type[0] == 'LayerNorm2x':
                norm1 = LayerNorm2x(dim, LayerNorm_type)
                norm1_2 = LayerNorm2x(dim, LayerNorm_type)
            elif norm_type[0] == 'LayerNorm2':
                norm1 = LayerNorm(dim*2, LayerNorm_type)
                norm1_2 = LayerNorm(dim*2, LayerNorm_type)
            elif norm_type[0] == 'LayerNorm_mu_sigma':
                norm1 = LayerNorm(dim, LayerNorm_type, True)
                norm1_2 = LayerNorm(dim, LayerNorm_type, True)
            elif norm_type[0] == 'BatchNorm':
                norm1 = nn.BatchNorm2d(dim)
                norm1_2 = nn.BatchNorm2d(dim)
            elif norm_type[0] == 'Softmax':
                norm1 = nn.Softmax(dim=1)
                norm1_2 = nn.Softmax(dim=1)
            else:
                norm1 = nn.Identity()
                norm1_2 = nn.Identity()
        else:
        ...
        if norm_type[1] == 'InstanceNorm':
            norm2 = nn.InstanceNorm2d(dim)
            norm2_2 = nn.InstanceNorm2d(dim)
        elif norm_type[1] == 'LayerNorm':
            norm2 = LayerNorm(dim, LayerNorm_type)
            norm2_2 = LayerNorm(dim, LayerNorm_type)
        elif norm_type[1] == 'LayerNorm2':
            norm2 = LayerNorm(dim*2, LayerNorm_type)
            norm2_2 = LayerNorm(dim*2, LayerNorm_type)
        elif norm_type[1] == 'LayerNorm_mu_sigma':
            norm2 = LayerNorm(dim, LayerNorm_type, True)
            norm2_2 = LayerNorm(dim, LayerNorm_type, True)
        elif norm_type[1] == 'BatchNorm':
            norm2 = nn.BatchNorm2d(dim)
            norm2_2 = nn.BatchNorm2d(dim)
        else:
            norm2 = nn.Identity()
            norm2_2 = nn.Identity()
        self.norm1 = norm1
        self.norm1_2 = norm1_2
    ...
    def forward(self, x, x2):
        ...
        if 'LN_DCT' in self.cs:
            ...
            x_attn, x_attnz = self.attn(self.norm1(x_dct), self.norm1_2(x_dct2))
            
            x_dct = x_dct + x_attn
            x_dct2 = x_dct2 + x_attnz

            x = self.idct(x_dct)
            x_dct2 = self.idct2(x_dct2)

        x_norm2 = self.norm2(x)
        x = x + self.ffn(x_norm2)

        x2_norm2 = self.norm2_2(x_dct2)
        x2 = x2 + self.ffn2(x2_norm2)
        return x, x2
...
class TransformerBlock_2b_2in(nn.Module):
    def __init__(self, dim=32,
        ...
        shift_size_ = [0, 0]  # [0, window_size_dct // 2] # [0, 0]
        # print(cs, norm_type_, qk_norm)
        self.trans1 = TransformerBlock_2in(dim, num_heads, ffn_expansion_factor,
                                       bias, LayerNorm_type, window_size, window_size_dct1, num_k=num_k,
                                       shift_size=shift_size_[0], cs=cs[0], norm_type=norm_type_[0],
                                       qk_norm=qk_norm[0], temp_adj=temp_adj, ffn=ffn
                                       )
        self.trans2 = TransformerBlock_2in(dim, num_heads, ffn_expansion_factor,
                                       bias, LayerNorm_type, window_size, window_size_dct2, num_k=num_k,
                                       shift_size=shift_size_[1], cs=cs[1], norm_type=norm_type_[1],
                                       qk_norm=qk_norm[1], temp_adj=temp_adj, ffn=ffn
                                       )
        # self.conv_b = conv_bench(dim)
    def forward(self, x, r, file_name=None):
        x, r = self.trans1(x, r)
        x, r = self.trans2(x, r)

        # Save feature: Since this program runs multiple times, modify the filename each run to avoid overwriting previous files.
 
        if file_name is not None:
            # 为了避免在多次运行时覆盖上一次的特征图，将文件名加上时间戳并增加一个运行次数，这样每次运行都能保存特征图，不会覆盖
            import time
            if not hasattr(self, '_feature_save_count'):
                self._feature_save_count = 0
            self._feature_save_count += 1
            timestamp = time.strftime('%Y%m%d_%H%M%S')
            file_name_base = file_name
            file_name_with_time = f"{file_name_base}_{self._feature_save_count}_feature_{timestamp}.png"
            self.save_feauure(x, file_name_with_time)
            return x, r, file_name
        return x, r
...
class HC_Mlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        # self.deep_conv = DeepConvBlock(hidden_features, hidden_features)
...
class CAB(nn.Module):
    ...
    def __init__(self, num_feat, compress_ratio=3, squeeze_factor=8):
...
class TSFN(nn.Module):
    def __init__(self,
                 ...
                 out_channels=3,
                 dim=32,
                 enc_blk_nums=[1, 1, 1],  # TODO
                 middle_blk_num=7,         # TODO
                 dec_blk_nums=[1, 1, 1],
                 heads_enc=[1, 2, 4],
                 heads_mid=8,
        ...
        TransformerBlockx = TransformerBlock_2b_2in
        ...
        if not self.return_feat:
            self.output_R = HC_Mlp(in_features=int(dim * 2), out_features=int(dim ** 1))
            self.merger = CAB(int(dim ))
        ...
        self.reduce_chan_ref = nn.ModuleList()
        chan = dim
        ...
        #         self.reduce_chan_ref.append(nn.Conv2d(int(chan), int(chan // 2), kernel_size=1, bias=bias))
        #         chan = chan // 2
        #     else:
        #         self.reduce_chan.append(nn.Identity())
        #         self.reduce_chan_ref.append(nn.Identity())
    ...
    def return_output(self, x, inp_img_, r, ref_img_, add_inp=False):
        if self.out_method == 'fourier_select':
            x = self.output(x, inp_img_)
        else:
            res = x.clone()
            x = torch.cat([x, r], dim=1)
            x = self.output_R(x) + res
            x = self.merger(x)
            x = self.output(x)
            if add_inp:
                x = x + inp_img_
    ...
    def forward(self, inp_img, ref_img, file_name=None):
        # print(inp_img.shape)
        ...
        if self.train_size and not self.grid:
            ...
            ref_img_, batch_list_ref = self.winp(ref_img)
        elif self.train_size and self.grid:
            inp_img_ = self.grids(inp_img)
            ref_img_ = self.grids(ref_img)
        else:  # self.train_size and not self.grid:
            inp_img_ = self.check_image_size(inp_img)
            ref_img_ = self.check_image_size(ref_img)
        # else:
        #     inp_img_ = inp_img
        h, w = inp_img_.shape[-2:]
        inp_enc_level1 = self.patch_embed(inp_img_)  # ShallowEncoder
        ref_enc_level1 = self.patch_embed2(ref_img_)  # ShallowEncoder
        ...
        # 保存每一次order的特征图
        if file_name is not None:
            self.save_feauure(x, file_name + '_enc_level0')
        ...
        #     file_name_tmp = file_name + '_enc_level_' + str(len(encs))
        #     if file_name is not None:
        #         self.save_feauure(x, file_name_tmp)

            # x, r = down(x, r)
        # x = self.dct_mid(x)
        if file_name is not None:
            x, r, _ = self.middle_blks(x, r, file_name)
        else:
            x, r = self.middle_blks(x, r, file_name)
        # x = self.idct_mid(x)

        # outputs_tmp =
        # for decoder, up, enc_skip, ref_enc_skip, reduce_ch, reduce_ch_ref in zip(self.decoders, self.ups, encs[::-1], ref_encs[::-1], self.reduce_chan, self.reduce_chan_ref):
        #     x, r = up(x, r)
        #     x = torch.cat([x, enc_skip], dim=1)
        #     r = torch.cat([r, ref_enc_skip], dim=1)
        #     x = reduce_ch(x)
        #     r = reduce_ch_ref(r)
        #     # x = dctx(x)
        #     # print(x.shape)
        #     x, r = decoder(x, r)
            ...
            # x = idctx(x)
        if file_name is not None:
            self.save_feauure(x, file_name + '_middle_blks_feature')
        # print(inp_img_.shape, x.shape)
        #### For Dual-Pixel Defocus Deblurring Task ####
        if self.dual_pixel_task:  # False
            x = x + self.skip_conv(inp_enc_level1)
            # r = r + self.skip_conv2(ref_enc_level1)
            if not self.return_feat:
                x = self.return_output(x, inp_img_, r, ref_img_, False)
                # r = self.return_output(r, ref_img_, False)
        ###########################
        else:
            if not self.return_feat:   # not False
                x = self.return_output(x, inp_img_, r, ref_img_, False)
                # r = self.return_output(r, ref_img_, False)
        ...
        if self.out_method == 'fourier_select':
            ...
            x = x[:, :, :H, :W].contiguous() + inp_img   # 直接这个
            # r = r[:, :, :H, :W].contiguous() + inp_img
...
if __name__=='__main__':
    ...
    net = TSFN(dim=dim, enc_blk_nums=enc_blk_nums, middle_blk_num=middle_blk_num,dec_blk_nums=dec_blk_nums,
