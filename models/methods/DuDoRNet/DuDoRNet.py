...
class DRDN(nn.Module):
    ...
    def test(self):
        inp = torch.randn(1, 2, 256, 256)
        k = torch.randn(1, 2, 256, 256)
        mask = torch.randn(1, 2, 256, 256)
        out = self.forward(inp,)
        from thop import profile
        flops, params = profile(self, inputs=(inp,))
...
if __name__ == '__main__':
    net = DRDN(n_channels=2, G0=32, kSize=3, D=3, C=4, G=32, dilateSet=[1,2,3,3])
    net.test()
    print('done')
